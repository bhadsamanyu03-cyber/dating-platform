...
def upgrade():
    op.create_table(
        ...
        sa.CheckConstraint(
            "maximum_distance_km BETWEEN 1 AND 10000", name="ck_discovery_preferences_distance"
        ),
        sa.CheckConstraint(
            "preferred_gender IN ('All', 'Woman', 'Man', 'Non-binary', 'Other', 'Prefer not to say')",
            name="ck_discovery_preferences_gender",
        ),
    )
