...
def payload(**overrides) -> ProfileUpdate:
    values = {"username": "valid_user", "display_name": "Valid", "bio": "About me", "gender": "Woman", "date_of_birth": "2000-01-01", "interest_ids": []}
    values.update(overrides)
    return ProfileUpdate(**values)


def test_username_and_bio_validation() -> None:
    with pytest.raises(ValidationError): payload(username="not valid")
    with pytest.raises(ValidationError): payload(bio="x" * 151)
