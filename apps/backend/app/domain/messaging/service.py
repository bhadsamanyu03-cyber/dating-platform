...
from app.domain.messaging.schemas import ConversationPage, ConversationResponse, MessageCreate, MessagePage, MessageResponse
...
def encode_cursor(message: Message) -> str:
    return base64.urlsafe_b64encode(json.dumps([message.created_at.isoformat(), str(message.id)]).encode()).decode()
...
class MessagingService:
    ...
    async def list(self, user_id: UUID):
        return ConversationPage(conversations=[ConversationResponse(id=x.id, match_id=x.match_id, created_at=x.created_at) for x in await self.repo.list_for_user(user_id, 50)], next_cursor=None)

    def message_response(self, value: Message):
        return MessageResponse(id=value.id, sender_user_id=value.sender_user_id, message_type=value.message_type,
            text_content=value.text_content, media_asset_ids=[link.media_asset_id for link in sorted(value.media, key=lambda x: x.ordering)],
            created_at=value.created_at, delivered_at=value.delivered_at, read_at=value.read_at,
            deleted_at=value.deleted_at, client_message_id=value.client_message_id)
    ...
    async def send(self, conversation_id: UUID, user_id: UUID, payload: MessageCreate):
        ...
        message = await self.repo.add_message(Message(conversation_id=conversation_id, sender_user_id=user_id,
            message_type="IMAGE" if payload.media_asset_ids else "TEXT", text_content=payload.text_content,
            media_asset_id=payload.media_asset_ids[0] if payload.media_asset_ids else None, client_message_id=payload.client_message_id), payload.media_asset_ids)
    ...
    async def messages(self, conversation_id: UUID, user_id: UUID, cursor: str | None, limit: int):
        ...
        await self.db.commit()
        return MessagePage(messages=[self.message_response(x) for x in page], next_cursor=encode_cursor(page[-1]) if extra and page else None)
