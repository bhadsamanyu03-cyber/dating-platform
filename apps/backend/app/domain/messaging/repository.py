...
class MessagingRepository:
    ...
    async def ensure_conversation(self, match_id: UUID) -> Conversation:
        result = await self.db.execute(
            insert(Conversation).values(match_id=match_id).on_conflict_do_nothing(
    ...
    async def conversation_for_user(self, conversation_id: UUID, user_id: UUID):
        return await self.db.scalar(
            select(Conversation).join(Match, Match.id == Conversation.match_id).where(
    ...
    async def list_for_user(self, user_id: UUID, limit: int):
        return list((await self.db.scalars(
            select(Conversation).join(Match).where(
                or_(Match.user_one_id == user_id, Match.user_two_id == user_id)
    ...
    async def media(self, asset_ids: list[UUID], owner: UUID):
        if not asset_ids:
            return []
        assets = list((await self.db.scalars(select(MediaAsset).where(
            MediaAsset.id.in_(asset_ids), MediaAsset.owner_user_id == owner,
            MediaAsset.upload_status == "UPLOADED", MediaAsset.media_type == "IMAGE",
        ))).all())
        return assets

    async def existing_message(self, sender_id: UUID, client_message_id: UUID):
        return await self.db.scalar(select(Message).options(selectinload(Message.media)).where(
    ...
    async def add_message(self, message: Message, asset_ids: list[UUID]):
        ...
        for ordering, asset_id in enumerate(asset_ids):
            self.db.add(MessageMedia(message_id=message.id, media_asset_id=asset_id, ordering=ordering))
        await self.db.flush()
        return await self.get_message(message.id)

    async def get_message(self, message_id: UUID):
        return await self.db.scalar(select(Message).options(selectinload(Message.media)).where(Message.id == message_id))

    async def messages(self, conversation_id: UUID, cursor: tuple[datetime, UUID] | None, limit: int):
        query = select(Message).options(selectinload(Message.media)).where(Message.conversation_id == conversation_id)
        if cursor:
        ...
        return list((await self.db.scalars(query.order_by(Message.created_at.desc(), Message.id.desc()).limit(limit + 1))).all())
    ...
    async def mark_read(self, conversation_id: UUID, reader_id: UUID):
        ...
        await self.db.execute(update(Message).where(
            Message.conversation_id == conversation_id, Message.sender_user_id != reader_id,
            Message.deleted_at.is_(None), Message.read_at.is_(None)
        ).values(read_at=now, delivered_at=now))

    async def asset_for_message(self, message_id: UUID, asset_id: UUID):
        return await self.db.scalar(select(MediaAsset).join(MessageMedia).where(
            MessageMedia.message_id == message_id, MessageMedia.media_asset_id == asset_id
        ))
